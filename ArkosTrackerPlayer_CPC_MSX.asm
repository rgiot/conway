...
;	Arkos Tracker Player V1.0 - CPC & MSX version.
;	21/09/09
...
;	*** MSX version hasn't been tested. Please contact us if it doesn't work. ***
...
PLY_Play
	...
	ld (PLY_PSGReg13_Retrig),a	;Reset the IsRetrig? flag.
...
PLY_PSGReg13_Code
	...
	out (#a0),a
	ld a,(hl)
PLY_PSGReg13_OLD cp 255			;Don't send the value if the same was previously sent.
PLY_PSGReg13_Retrig nop			;If IsRetrig?, this is modified to 'CP xx' (#fe), so that the RET Z following isn't executed.
	ret z

	out (#a1),a
	ld (PLY_PSGReg13_OLD + 1),a
	ret
